...
function pick(guess) {
  ...
  while (numbers.length < 10) {
    if (LPC25++ % 2000 === 0 && Date.now() - LP25 > 1500) {
      console.log("Potential infinite loop detected on line " + 269 + ". Tests may be failing because of this.");
      break;
    }
  ...
  const LP26 = Date.now();
  for (let i = 0; i < 10; i++) {
    if (LPC26++ % 2000 === 0 && Date.now() - LP26 > 1500) {
      console.log("Potential infinite loop detected on line " + 273 + ". Tests may be failing because of this.");
      break;
    }
